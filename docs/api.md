...
| **POST** | `/publisher/submit-orders?count=N` | Submit `N` orders per symbol with alternating BUY/SELL sides and limit prices that cross both the bootstrap spread (99.50/100.50) and the tighter production spread (99.95/100.05). Body: `List<String>` of symbols. Returns an `Integer` count of orders the exchange accepted (HTTP 200); non-200 responses are logged at `WARN` level and not counted (stale quotes / empty residual depth legitimately reject), and the first such failure in the wave is remembered for `/publisher/last-failure`. |
| **GET** | `/publisher/last-failure` | Returns a plain-text summary (`HTTP <status> <body>` or `EXCEPTION <type>: <msg>`) of the first non-200 / exception that the most recent `submit-orders` wave saw. Empty string if the publisher has only seen 200s since startup. Used by the error-case-11 end-to-end test to surface the exchange-side reason for an unexpected zero-accepted wave directly in the JUnit assertion message instead of forcing the operator to scrape pod logs. |
| **GET** | `/health` | Returns `ServiceHealth`. |
